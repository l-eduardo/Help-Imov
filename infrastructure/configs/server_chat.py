    ...
    try:
        ...
        async for message in websocket:
            ...
            ##################################################################################
                ...
                if filtered_users:
                    await curr_user.ws.send(
                        json.dumps({'type': 'change-my-name', 'status': 'no', 'message': 'name is taken'}))
                    continue
            ...
            #                                         |___/
            # ON
            elif data["action"] == "update_my_ascii_frame":
                curr_user.curr_ascii_img = data["ascii_img"]
            # OFF
            elif data["action"] == "close_my_ascii_frame":
                curr_user.curr_ascii_img = ''
            # SEND
                ...
                if ascii_img:
                    await curr_user.ws.send(
                        json.dumps({'type': 'view_ascii_frame', 'status': 'ok', 'ascii_img': ascii_img}))
                else:
